...
public class LocationCache {
	...
	Connection myConn = DriverManager.getConnection("jdbc:mysql://localhost/homeDB", "root","home123");
	...
	ResultSet myRs = myStmt.executeQuery("select streetaddr,zipcode,locationAccuracy  from locationCache where latitude='"+llz.getLatitude()+"' and longitude='"+llz.getLongitude()+"'");
	...
	public static void ifAlreadyExists_addr(LLZ llz) throws SQLException{
		...
		Statement myStmt = myConn.createStatement();
		String add=llz.getAddress().replace("'", "\\'");
	
	//	ResultSet myRs = myStmt.executeQuery("select * from crimesLocCache where streetaddr='"+add+"' and state='"+llz.getState()+"' and city='"+llz.getCity()+"'");
		ResultSet myRs = myStmt.executeQuery("select * from locationCache where streetaddr='"+add+"' and state='"+llz.getState()+"'");
...
//		String query2 = "INSERT INTO `homeDB`.`locationCache` (latitude ,longitude ," +
...
//System.out.println("insert in process");
		System.out.printf(" insertInto %d ",llz.getMyId());
